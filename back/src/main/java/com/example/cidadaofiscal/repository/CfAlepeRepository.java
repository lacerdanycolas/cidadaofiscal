	 ...
	 value = "SELECT \n" +
	 		...
	 		@Query(
		 		value="SELECT\n" +
		 				...
		 				"    cidadaofiscal.cf_alepe\n" +
		 				"WHERE \n" +
		 				"LOWER(parlamentar_fantasia) LIKE LOWER(CONCAT('%',:name, '%')) \n" +
		 				"AND LOWER(parlamentar_partido) LIKE LOWER(CONCAT('%',:partido, '%')) \n" +
		 				"AND LOWER(parlamentar_nome) LIKE LOWER(CONCAT('%',:name, '%')) \n" +
		 				"AND ordem_ano IS NOT NULL\n" +
		 				"AND ordem_mes IS NOT NULL\n" +
		 				"AND fornecedor_id IS NOT NULL\n" +
		 				"AND LOWER(fornecedor_nome) LIKE LOWER(CONCAT('%',:fornecedor, '%')) \n" +
		 				"AND despesa_tipo IS NOT NULL\n" +
		 				"AND despesa_valor BETWEEN :valorMin AND :valorMax \n" +
		 				"AND despesa_cancelada IS NOT NULL\n" +
		 				"AND despesa_data BETWEEN :dataMin AND :dataMax \n" +
		 				"ORDER BY despesa_data ASC\n" +
		 				"\n" +
		 				"\n" +
		 				"\n" +
		 				"", nativeQuery = true)
		 				 List<Object[]> getDeputadosFiltro(@Param("name") String name,
		 						@Param("partido") String partido, @Param("fornecedor") String fornecedor,
		 						@Param("valorMin") String valorMin, @Param("valorMax") String valorMax,
		 						@Param("dataMin") String dataMin, @Param("dataMax") String dataMax);
